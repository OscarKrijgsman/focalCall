...
package named, FocalCall. FocalCall permits user-defined size cut-offs to recognize focal 
aberrations and builds on established array CGH segmentation and calling algorithms. 
...
\citep{Bierkens} and preprocessed using \citep{CGHcall}. The example set used here only contains complete chromosome 2. 
...
\pagebreak
\noindent
Alternatively, we can generate files for visualisation in IGV with {\tt igvFiles}. The files will be written 
to the home directory and can be loaded in IGV directly. 
